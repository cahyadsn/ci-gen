...
class Gen extends CI_Controller {
    ...
    public function __construct()
    {
        ...
        $this->config->load('gen',TRUE);
        $this->_templates_loc = APPPATH.$this->config->item('templates', 'gen');
        $this->_c_extends = $this->config->item('c_extends', 'gen');
        $this->_mo_extends = $this->config->item('mo_extends', 'gen');
        $this->_mi_extends = $this->config->item('mi_extends', 'gen');
    ...
    public function create_migration()
    {
        ...
        if(isset($action))
        {
            ...
            else
            {
                ...
                {
                    $pattern = '/[0-9]{3}/';
                    if(preg_match($pattern, $migration, $gens))
                    {
                        $migration_version = intval($gens[0]);
